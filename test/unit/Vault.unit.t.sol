...
// VaultUnitTest - Comprehensive Unit Tests for Vault.sol
// @title VaultUnitTest
// @notice Comprehensive unit tests covering all Vault functions with success and revert paths
// @dev Tests deposit(), fulfil(), settle(), verifyRequestSignature(), extractAddress()
//      and access control. All require statements are tested.
contract VaultUnitTest is BaseVaultTest {
    ...
    /// @notice Test fulfilling with multiple destinations
    function test_Fulfil_MultipleDestinations_Success() public {
        uint256 nonce = 12;
        uint256 expiry = _futureTimestamp(1 hours);
    ...
    function test_VerifyRequestSignature_Valid_ReturnsTrue() public {
        uint256 depositAmount = 1 ether;
        uint256 nonce = 30;
        uint256 expiry = _futureTimestamp(1 hours);
        ...
        (bool success, bytes32 signedMessageHash) = vault.verifyRequestSignature(request, signature);
        
        assertTrue(success, "Valid signature should return true");
        assertTrue(signedMessageHash != bytes32(0), "Signed message hash should be non-zero");
    }
    
    /// @notice Test invalid signature returns false
    function test_VerifyRequestSignature_Invalid_ReturnsFalse() public {
        uint256 depositAmount = 1 ether;
        uint256 nonce = 31;
        uint256 expiry = _futureTimestamp(1 hours);
        ...
        (bool success, ) = vault.verifyRequestSignature(request, wrongSignature);
        
        assertFalse(success, "Invalid signature should return false");
    }
    
    /// @notice Test empty signature reverts
    function test_VerifyRequestSignature_Empty_Reverts() public {
        uint256 depositAmount = 1 ether;
        uint256 nonce = 32;
        uint256 expiry = _futureTimestamp(1 hours);
        ...
        // Empty signature causes ECDSAInvalidSignatureLength revert
        vm.expectRevert();
        vault.verifyRequestSignature(request, emptySignature);
    }
    
    /// @notice Test malformed signature reverts
    function test_VerifyRequestSignature_Malformed_Reverts() public {
        uint256 depositAmount = 1 ether;
        uint256 nonce = 33;
        uint256 expiry = _futureTimestamp(1 hours);
        ...
        // Malformed signature causes ECDSAInvalidSignatureLength revert
        vm.expectRevert();
        vault.verifyRequestSignature(request, malformedSignature);
    }

    // extractAddress() Tests (via deposit/fulfil)
    ...
    function test_Settle_VerifierRole_Success() public {
        // This is tested in test_Settle_SingleSolverETH_Success
        // The verifier key is granted SETTLEMENT_VERIFIER_ROLE in setUp
        assertTrue(vault.hasRole(SETTLEMENT_VERIFIER_ROLE, verifier), "Verifier should have role");
    }
