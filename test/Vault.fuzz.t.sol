...
contract VaultFuzzTest is BaseVaultTest {
    ...
    function testFuzz_Deposit_ERC20(
        uint256 depositAmount,
        uint256 nonce,
        uint256 expiryOffset
    ) public {
    ...
    function testFuzz_Fulfil_ETH(
        ...
        assertEq(
            vault.winningSolver(signedMessageHash),
            solver,
            "Winning solver should be recorded"
        );
        
        assertEq(
            requester.balance - recipientBalanceBefore,
            fulfilAmount,
    ...
    /// @param nonce Unique nonce
    /// @param expiryOffset Seconds until expiry
    function testFuzz_Fulfil_ERC20(
        uint256 depositAmount,
        uint256 fulfilAmount,
        uint256 nonce,
        uint256 expiryOffset
        ...
        assertEq(
            token.balanceOf(requester) - recipientBalanceBefore,
            fulfilAmount,
    ...
    /// @param settleAmount Amount to settle (bounded)
    /// @param nonce Unique nonce
    function testFuzz_Settle_ERC20(
        uint256 settleAmount,
        uint256 nonce
    ) public {
        // Bound inputs
    ...
    /// @param nonce Unique nonce
    function testFuzz_Settle_MultipleSolvers(
        uint256 amount1,
        uint256 amount2,
        uint256 nonce
    ) public {
        ...
        assertTrue(vault.settleNonce(nonce), "Settle nonce should be marked as used");
        assertEq(
            solver.balance - solver1BalanceBefore,
            amount1,
            "Solver 1 should receive correct amount"
        );
        assertEq(
            solver2.balance - solver2BalanceBefore,
            amount2,
            "Solver 2 should receive correct amount"
        );
    }
}
